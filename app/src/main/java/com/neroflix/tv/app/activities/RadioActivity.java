...
import com.neroflix.tv.app.services.RadioPlayerService;
...
public class RadioActivity extends AppCompatActivity {
    ...
    private final ServiceConnection serviceConn = new ServiceConnection() {
        ...
        public void onServiceConnected(ComponentName name, IBinder binder) {
            RadioPlayerService.LocalBinder lb = (RadioPlayerService.LocalBinder) binder;
            radioService = lb.getService();
            serviceBound = true;
            updatePlayPauseBtn();
        }
        @Override
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Bind to service (starts it too)
        Intent svcIntent = new Intent(this, RadioPlayerService.class);
            ...
            try {
                ...
                try (Response resp = client.newCall(req).execute()) {
                    if (!resp.isSuccessful() || resp.body() == null) {
                        mainHandler.post(() -> showError("Failed (HTTP " + resp.code() + ")"));
                        return;
                    }
    ...
    private void onPlaylistLoaded(List<M3UParser.Channel> result) {
        ...
        if (result.isEmpty()) { statusText.setText("No stations found"); return; }
    ...
    private void playChannel(int index) {
        ...
        if (!TextUtils.isEmpty(ch.logo)) {
            Glide.with(this).load(ch.logo)
                    .placeholder(R.drawable.ic_radio).error(R.drawable.ic_radio)
                    .into(stationLogo);
        } else {
        ...
        intent.putExtra(RadioPlayerService.EXTRA_URL,    ch.url);
        intent.putExtra(RadioPlayerService.EXTRA_NAME,   ch.name);
        intent.putExtra(RadioPlayerService.EXTRA_GROUP,  ch.group);
        intent.putExtra(RadioPlayerService.EXTRA_IS_HLS, ch.isHls);
        startService(intent);
    ...
    private void togglePlayPause() {
        if (serviceBound && radioService != null) {
            ...
            // Small delay then refresh button
            mainHandler.postDelayed(this::updatePlayPauseBtn, 150);
        }
    }

    private void updatePlayPauseBtn() {
        if (playPauseBtn == null) return;
        boolean playing = serviceBound && radioService != null && radioService.isPlaying();
    ...
    protected void onDestroy() {
        ...
        executor.shutdown();
        // NOTE: Service keeps running for background audio
    }

    // ── Inline Adapter ────────────────────────────────────────────────────

    public static class RadioChannelAdapter extends RecyclerView.Adapter<RadioChannelAdapter.VH> {
        ...
        public void setPlaying(int pos) { int old = playingPos; playingPos = pos; if (old >= 0) notifyItemChanged(old); notifyItemChanged(pos); }
        public void setFocused(int pos) { int old = focusedPos; focusedPos = pos; if (old >= 0) notifyItemChanged(old); notifyItemChanged(pos); }
        ...
        public void onBindViewHolder(@NonNull VH h, int pos) {
            ...
            h.itemView.setBackgroundColor(playing ? 0x33E50914 : (focused ? 0x22FFFFFF : 0x00000000));
